...
//        tf.backgroundColor = UIColor.red
    ...
    @objc func handleSendButton (){
        ...
        APIClient.sendMessage(message: ms.text ?? "", fromId: auth.uid, name: auth.displayName ?? "", courseId: courseID)
            .do(onError: { [weak self](error) in
                self?.showError(message: "Đã có lỗi sảy ra hãy thử lại.")
            })
    ...
    func observeValueMessage(){
        ...
        ref.child("Messenger").child("Course_\(self.courseID)").observe(.childAdded, with: { (snap) in
            if let value = snap.value as? NSDictionary {
                ...
                mess.isSender = value["isTeacher"] as! Int != 0
...
extension ChatLogCollectionViewController: StoryboardSceneBased {
    static var sceneStoryboard = UIStoryboard(name: "Chat", bundle: nil)
}
